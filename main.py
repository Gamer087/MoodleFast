...
def processUploadFiles(filename,filesize,files,update,bot,message,thread=None,jdb=None):
    try:
        bot.editMessageText(message,'🗂Preparando subida☁️...')
        evidence = None
        fileid = None
        ...
        if cloudtype == 'moodle':
            ...
            loged = client.login()
            bot.editMessageText(message,'🌩Subiendo Porfavor Espere⚡️...')
            if loged:
               originalfile = ''
               if len(files)>1:
                    originalfile = filename
...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if username in str(tl_admin_user).split(';') or user_info :  # validate user
            ...
            msg323= "🚫No tienes acceso al bot, contacta con mi 🖥desarrollador: @TuguerX"
        ...
        if '/ban' in msgText:
            ...
            if isadmin:
                try:
                    user = str(msgText).split(' ')[1]
                    if user == username:
                        bot.sendMessage(update.message.chat.id,'😑No Se Puede Banear Usted mismo😑')
                        return
                    jdb.remove(user)
                    jdb.save()
        ...
        if '/evea' in msgText:
            ...
            if getUser:
                ...
                bot.sendMessage(update.message.chat.id,f"✅Configuración de {hostmo} aplicada con exito")
            return
        if '/eva' in msgText:
            getUser = user_info
            user = ''
        ...
        if '/setproxy' in msgText:
            ...
            if getUser:
                try:
                   ...
                   bot.sendMessage(update.message.chat.id,'⚠️Error al equipar proxy🔌.')
                return
        if '/info' in msgText:
            getUser = user_info
            if getUser:
        ...
        if '/zips' in msgText:
            ...
            if getUser:
                try:
                   ...
                   bot.sendMessage(update.message.chat.id,'⚠️Error al cambiar los zips: '+str(ex))
                return
        if '/acc' in msgText:
            try:
        ...
        if '/rename_on' in msgText:
            try:
                ...
                if getUser:
                    ...
                    jdb.save()
                    statInfo = infos.createStat(username,getUser,jdb.is_admin(username))
                    bot.sendMessage(update.message.chat.id,"✅autonombrado activado.")
        ...
        if '/rename_off' in msgText:
            try:
                ...
                if getUser:
                    ...
                    jdb.save()
                    statInfo = infos.createStat(username,getUser,jdb.is_admin(username))
                    bot.sendMessage(update.message.chat.id,"❌autonombrado desactivado.")
        ...
        if '/nube' in msgText:
            try:
                ...
                else: bot.sendMessage(update.message.chat.id,"⚠️Tipo de nube no permitido.")     
            except:
            ...
            try:
                ...
                if "calendar" == type or "evidence" == type or "perfil" == type or "draft" == type or "blog" == type:   
                 ...
                 if getUser:
                    ...
                    jdb.save()
                    statInfo = infos.createStat(username,getUser,jdb.is_admin(username))
                    bot.sendMessage(update.message.chat.id,f"📤Tipo de subida cambiado a: {type}")
                else: bot.sendMessage(update.message.chat.id,f"⚠️Tipo de subida no permitido📤")      
            except Exception as ex:
        ...
        if '/set_proxy' in msgText:
            try:
               ...
               else: bot.sendMessage(update.message.chat.id,"⚠️Proxy no permitido, debe llevar lo siguiente: socks5://")           
            except: pass
            return
        if '/cript' in msgText:
        ...
        if '/del_files' in msgText and user_info['cloudtype']=='moodle':
            ...
            try:
                ...
                bot.sendMessage(update.message.chat.id,'⚠️No se pudieron eliminar 50 elementos solo se eliminaron '+str(eliminados))
